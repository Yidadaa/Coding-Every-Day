...
\end{multicols}
		...
		\item \cvcompany{三维视觉中的深度估计算法研究 @ 未来媒体研究中心}{from 2019.04}\cvsublevel{%
			...
			\cvsubbullet{%
				当前的想法：现有的无监督训练流程有着训练速度过慢的缺点，尝试从以下几个方面改进：Co-attention 模块已被证明可以很好地挖掘相邻视频帧中%
				具有较高相似度的特征区域，所以尝试在特征图层面对相邻帧的特征进行 attention，使得相似度较高的区域产生更接近的深度估计结果；%
				其次目前无监督流程中的 PoseNet 部分预测出的相机运动存在抖动问题，所以提出%
				使用贝塞尔曲线对 PoseNet 的输出做平滑约束，减少相机抖动带来的影响，从而加快无监督训练流程。
			}
			\cvsubbullet{%
				\textbf{预投 IJCAI}
			}
		...
		\item \cvcompany{基于 Unity 虚拟环境和强化学习的机械臂控制算法 @ 毕业设计}{2017.10 - 2018.06}\cvsublevel{%
			\cvsubbullet{强化学习算法在机械智能控制中愈发重要，然而强化学习算法往往需要在训练阶段通过大量地 exploration 与环境交互取得数据来优化策略，这种训练策略将会在
				真实世界带来极高的时间和物料成本，本文提出了一种架构，可以在 Unity 中构建尽可能贴近真实环境的虚拟环境，为迁移至真实环境提供预训练基础。
			}
			\cvsubbullet{本文主要工作如下：1) 构建了一个沟通 Unity 运行时和 Python 运行时的中间层，使得基于 Tensorflow 和 Pytorch 的构建的
				深度学习模型可以与 Unity 虚拟环境交互；2) 实现了强化学习中的经典算法 PPO 算法，并分别在二维和三维环境中设计对应的任务来验证算法和中间层的可用性；
